...
public class GenericSourceTest extends AndroidTestCase {
    ...
    NetworkRoeDao<DummyObject, Integer> dummyDao;

    public void setUp() throws Exception {
        GenericDatabase.getInstance(getContext()); //initialize database
        dummyDao = GenericDatabase.getInstance().getDao(DummyObject.class);
    ...
    public void testBasicSingleOperationsWithNetwork() throws Exception {
        ...
        assertTrue("new object should not have a local id", d.getId() == -1);
        assertTrue("new object should not have a server id", d.getServerId() == -1);
...
//        assertEquals("saved object should be the same instance", d, saveResult);
        assertFalse("saved object should have a local id", d.isNew());
        assertEquals("should be 1 item in database", 1, dummyDao.countOf());
        assertNotNull("item should be in database", dummyDao.queryForId(d.getId()));

        assertEquals("server should have seen create", serverCreateCount + 1, DummyObjectMockServer.getInstance().createCount);
        assertTrue("saved object should be synced", d.hasServerId());
        assertTrue("saved object should have a server id", d.getServerId() > -1);

        //READ
        DummyObject readResult = dummyDao.queryForId(d.getId());
...
//        assertEquals("updated object should have the same server id", d.getServerId(), updateResult.getServerId());
        assertEquals("updated object should have the new values", 10, d.getAge());
        assertEquals("should still be 1 item in database", 1, dummyDao.countOf());
...
//        assertEquals("deleted item should be the same item", d, deleteResult);
        assertNull("item should no longer be in database", dummyDao.queryForId(d.getId()));
        assertEquals("should still be no items in database", 0, dummyDao.countOf());
...
//    }
    ...
    public void testObservablesOnBasicSingleOperationsWithNetwork() throws Exception {
        ...
        dummyDao.addObserver(d, catcher1);
        dummyDao.addObserver(d, catcher2);
...
//    }
    ...
    public void testSourceWithoutServer() throws Exception {
        ...
        RoeDao<NoNetworkResource, Integer> nnrDao = GenericDatabase.getInstance().getDao(NoNetworkResource.class);

        nnrDao.createOrUpdate(resource);
        ...
        assertEquals("deleting locally should not have created an unsynced resoure row", 0, GenericDatabase.getInstance().getDao(UnsyncedResource.class).countOf());
    }
